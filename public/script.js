...
async function fetchTrafficData(city) {
  const url = `https://traffic.api.here.com/traffic/6.3/incidents.json?app_id=YOUR_APP_ID&app_code=YOUR_APP_CODE&bbox=${city.bbox}&criticality=critical`;
  try {
    const response = await fetch(url);
    const data = await response.json();
...
async function fetchDisasterData() {
  ...
  try {
    const response = await fetch(url);
    const data = await response.json();
    // Process and display disaster data
  } catch (error) {
...
async function fetchTravelData(city) {
  const url = `https://api.travelpayouts.com/v1/prices/cheap?origin=${city}&destination=ANY&token=YOUR_API_TOKEN`;
  try {
    const response = await fetch(url);
    const data = await response.json();
    // Process and display travel data
  } catch (error) {
...
async function sendMessageToChatbot(message) {
  try {
    ...
    const data = await response.json();
    chatbotResponse.innerText = data.reply;
  } catch (error) {
    chatbotResponse.innerText = 'Error communicating with chatbot';
  }
}
